...
public class TriadDeckOptimizer {
	...
	private ManualResetEvent loopPauseEvent = new(true);
	...
	private static int GetRandomSeed(int Idx0, int Idx1, int Idx2, int Idx3, int Idx4) {
		var Hash = 13;
		Hash = Hash * 37 + Idx0;
		Hash = Hash * 37 + Idx1;
		Hash = Hash * 37 + Idx2;
	...
	private struct CardScoreData : IComparable<CardScoreData> {
		...
		public int CompareTo(CardScoreData other) => -score.CompareTo(other.score);

		public override string ToString() => card.ToShortCodeString() + ", score: " + score;
	}
	...
	private bool FindCardPool(List<TriadCard> allCards, List<TriadGameModifier> modifiers, List<TriadCard> lockedCards) {
		...
		var ascentionModIdx = modifiersCopy.FindIndex(mod => mod.GetType() == typeof(TriadGameModifierAscention));
		var hasAscensionMod = ascentionModIdx >= 0;
		;

		var descentionModIdx = modifiersCopy.FindIndex(mod => mod.GetType() == typeof(TriadGameModifierDescention));
		var hasDescentionMod = descentionModIdx >= 0;
		;
		...
		var priRarityThr = new List<ETriadCardRarity>();
		...
		var commonScoredList = new List<CardScoreData>();
		var priScoredList = new List<List<CardScoreData>>();
		...
		foreach (var card in allCards) {
			...
			var scoredCard = new CardScoreData { card = card, score = card.OptimizerScore };
	...
	private class SlotIterator {
		...
		public struct ItemInfo {
			...
			public bool IsValid() => Cards[0] != Cards[1] && Cards[0] != Cards[2] && Cards[0] != Cards[3] && Cards[0] != Cards[4] &&
	...
	private void FindDecksScored(TriadGameModifier[] regionMods, List<TriadCard> lockedCards) {
		...
		do {
			var loopResult = Parallel.ForEach(slotIterator.GetDecks(lowestPauseIdx), options, (deckInfo, state) => {
				if (isPaused) { state.Break(); } else if (bAbort) { state.Stop(); }
	...
	public int GetSecondsRemaining(int ElapsedMs) {
		...
		var numSecRemaning = numTestedPerMs > 0 ? numTestsRemaning / numTestedPerMs / 1000 : numTestsRemaning * numMsPerTest / 1000;

...
public class UIStateTriadCard : IEquatable<UIStateTriadCard> {
	...
	public bool Equals(UIStateTriadCard? other) => other != null &&
	...
	public override string ToString() {
		...
		var desc = $"[{numU:X}-{numL:X}-{numD:X}-{numR:X}], owner:{owner}";
		if (isLocked) {
			desc += " (locked)";
		}
	...
	public TriadCard? ToTriadCard(GameUIParser ctx) => !isPresent ? null :

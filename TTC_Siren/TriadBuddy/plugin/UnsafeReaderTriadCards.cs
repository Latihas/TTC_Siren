...
public class UnsafeReaderTriadCards {
	...
	public UnsafeReaderTriadCards() {
		...
		try {
			...
			IsNpcBeatenPtr = Plugin.SigScanner.ScanText("40 53 48 83 ec 20 8d 82 fe ff dc ff");
			...
			IsCardOwnedPtr = Plugin.SigScanner.ScanText("40 53 48 83 ec 20 48 8b d9 66 85 d2 74 3b 0f");

			// UIState addr, use LEA opcode before calling IsTriadCardOwned, same function as described above
			UIStatePtr = Plugin.SigScanner.GetStaticAddressFromSig("48 8d 0d ?? ?? ?? ?? e8 ?? ?? ?? ?? 84 c0 74 0f 8b cb");
		} catch (Exception ex) {
		...
		if (!HasErrors) {
			...
			Plugin.Log.Error("Failed to find triad card functions, turning reader off");
		}
	}

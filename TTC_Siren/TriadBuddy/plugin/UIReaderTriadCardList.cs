...
public class UIReaderTriadCardList : IUIReader {
	...
	public void OnAddonShown(IntPtr addonPtr) {
		cachedAddonAgentPtr = addonPtr != IntPtr.Zero ? Plugin.GameGui.FindAgentInterface(addonPtr) : IntPtr.Zero;
	...
	public static unsafe IntPtr LoadFailsafeAgent() {
		var uiModule = (UIModule*)Plugin.GameGui.GetUIModule().Address;
		if (uiModule != null) {
	...
	public unsafe bool SetPageAndGridView(int pageIndex, int cellIndex) {
		...
		IntPtr addonPtr = Plugin.GameGui.GetAddonByName(GetAddonName());
	...
	private void SetStatus(Status newStatus) {
		if (status != newStatus) {
			...
			if (HasErrors) {
				Plugin.Log.Error("CardList reader error: " + newStatus);
			}

...
public class TriadCardDB {
	...
	public TriadCard? FindByTexture(string texPath) {
		...
		if (texPath != null && texPath.EndsWith(".tex")) {
			var pathPattern = "082000/082";

			var patternPos = texPath.IndexOf(pathPattern);
			if (patternPos > 0) {
				var idStr = texPath.Substring(patternPos + pathPattern.Length, 3);
				if (int.TryParse(idStr, out var cardId)) {
					if (cardId >= 500) { cardId -= 500; } else if (cardId >= 100) { cardId -= 100; } else { cardId = -1; }
			...
			var pathPatternBig = "087000/087";
			var pathPatternSmall = "088000/088";
	...
	public static uint GetCardTextureId(int cardId) => cardId < 0 || cardId >= 1000 ? 87000 : (uint)cardId + 87000;

	public static uint GetCardIconTextureId(int cardId) => cardId < 0 || cardId >= 1000 ? 88000 : (uint)cardId + 88000;

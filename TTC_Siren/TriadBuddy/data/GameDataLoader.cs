...
public class GameDataLoader {
		...
		Task.Run(async () => {
			...
			for (var retryIdx = 3; retryIdx >= 0; retryIdx--) {
				...
				try {
					...
					Plugin.Log.Warning(ex, "exception while parsing! retry:{0}", needsRetry);
				}

				if (needsRetry) {
					await Task.Delay(2000);
					Plugin.Log.Info("retrying game data parsers...");
				} else {
					break;
				}
			}
	...
	private void ParseGameData() {
		...
		if (result) {
			...
			Plugin.Log.Info($"Loaded game data for cards:{cardDB.cards.Count}, npcs:{npcDB.npcs.Count}");
			IsDataReady = true;
		} else {
	...
	private static bool ParseRules() {
		...
		var rulesSheet = Plugin.DataManager.GetExcelSheet<TripleTriadRule>();
		if (rulesSheet == null || rulesSheet.Count != modDB.mods.Count) {
			Plugin.Log.Fatal($"Failed to parse rules (got:{rulesSheet?.Count ?? 0}, expected:{modDB.mods.Count})");
			return false;
		}
	...
	private static bool ParseCardTypes() {
		var locDB = LocalizationDB.Get();

		var typesSheet = Plugin.DataManager.GetExcelSheet<TripleTriadCardType>();
		if (typesSheet == null || typesSheet.Count != locDB.LocCardTypes.Count) {
			Plugin.Log.Fatal($"Failed to parse rules (got:{typesSheet?.Count ?? 0}, expected:{locDB.LocCardTypes.Count})");
			return false;
		}
	...
	private static bool ParseCards() {
		var cardDB = TriadCardDB.Get();
		var cardInfoDB = GameCardDB.Get();

		var cardDataSheet = Plugin.DataManager.GetExcelSheet<TripleTriadCardResident>();
		var cardNameSheet = Plugin.DataManager.GetExcelSheet<TripleTriadCard>();

		if (cardDataSheet != null && cardNameSheet != null && cardDataSheet.Count == cardNameSheet.Count) {
			var cardTypesSheet = Plugin.DataManager.GetExcelSheet<TripleTriadCardType>();
			var cardRaritySheet = Plugin.DataManager.GetExcelSheet<TripleTriadCardRarity>();
			if (cardTypesSheet == null || cardTypesSheet.Count != cardTypeMap.Length) {
				Plugin.Log.Fatal($"Failed to parse card types (got:{cardTypesSheet?.Count ?? 0}, expected:{cardTypeMap.Length})");
				return false;
			}
			if (cardRaritySheet == null || cardRaritySheet.Count != cardRarityMap.Length) {
				Plugin.Log.Fatal($"Failed to parse card rarities (got:{cardRaritySheet?.Count ?? 0}, expected:{cardRarityMap.Length})");
				return false;
			}
			...
			for (uint idx = 0; idx < cardDataSheet.Count; idx++) {
				...
				if (rowData != null && rowName != null && rowData.Value.Top > 0) {
					...
					if (absDiff > 10) {
						Plugin.Log.Fatal($"Failed to assign card data (got:{cardDB.cards.Count}, expected:{idx})");
						return false;
					}
			...
			Plugin.Log.Fatal($"Failed to parse card data (D:{cardDataSheet?.Count ?? 0}, N:{cardNameSheet?.Count ?? 0})");
			return false;
		}
	...
	private bool ParseNpcs() {
		...
		var npcDataSheet = Plugin.DataManager.GetExcelSheet<TripleTriad>();
		...
		if (listTriadIds.Count == 0 || npcDataSheet == null) {
			Plugin.Log.Fatal("Failed to parse npc data (missing ids)");
			return false;
		}

		var mapTriadNpcData = new Dictionary<uint, NpcIds>();
		var sheetNpcNames = Plugin.DataManager.GetExcelSheet<ENpcResident>();
		var sheetENpcBase = Plugin.DataManager.GetExcelSheet<ENpcBase>();
		if (sheetNpcNames != null && sheetENpcBase != null) {
			...
			Plugin.Log.Fatal($"Failed to parse npc data (NN:{sheetNpcNames?.Count ?? 0}, NB:{sheetENpcBase?.Count ?? 0})");
			return false;
		}
		...
		foreach (var rowData in npcDataSheet) {
			...
			foreach (var ruleRow in rowData.TripleTriadRule) {
				if (ruleRow.RowId != 0 && ruleRow.IsValid) {
					if (ruleRow.RowId >= modDB.mods.Count) {
						Plugin.Log.Fatal($"Failed to parse npc data (rule.id:{ruleRow.RowId})");
						return false;
					}
					...
					if (ruleValueOb.Name.ToString() != logicRule.GetLocalizedName()) {
						Plugin.Log.Fatal($"Failed to match npc rules! (rule.id:{ruleRow.RowId})");
						return false;
					}
				}
			}
			...
			var cardsFixed = new int[5];
			{
				if (rowData.TripleTriadCardFixed.Count != 5) {
					Plugin.Log.Fatal($"Failed to parse npc data (num CF:{rowData.TripleTriadCardFixed.Count})");
					return false;
				}
				...
				for (var cardIdx = 0; cardIdx < rowData.TripleTriadCardFixed.Count; cardIdx++) {
					...
					if (cardRowIdx != 0) {
						if (cardRowIdx >= cardDB.cards.Count) {
							Plugin.Log.Fatal($"Failed to parse npc data (card.id:{cardRowIdx})");
							return false;
						}
			...
			var cardsVariable = new int[5];
			{
				if (rowData.TripleTriadCardVariable.Count != 5) {
					Plugin.Log.Fatal($"Failed to parse npc data (num CV:{rowData.TripleTriadCardVariable.Count})");
					return false;
				}
	...
	private bool ParseNpcAchievements() {
		var npcDataSheet = Plugin.DataManager.GetExcelSheet<TripleTriadResident>();
		if (npcDataSheet != null) {
			// rowIds are not going from 0 here!
			foreach (var rowData in npcDataSheet) {
	...
	private bool ParseNpcLocations() {
		var sheetLevel = Plugin.DataManager.GetExcelSheet<Level>();
		if (sheetLevel != null) {
		...
		var sheetMap = Plugin.DataManager.GetExcelSheet<Map>();
		if (sheetMap != null) {
	...
	private bool ParseCardRewards() {
		var sheetItems = Plugin.DataManager.GetExcelSheet<Item>();
		if (sheetItems != null) {
				...
				foreach (var itemId in kvp.Value.rewardItems) {
					...
					if (itemRow != null) {
						...
						if (cardOb != null) {
							...
							Plugin.Log.Error($"Failed to parse npc reward data! npc:{kvp.Value.triadId} ({npcName}), rewardId:{itemId} ({itemRow.Value.Name} | {itemRow.Value.Singular})");
						}
					}
				}
			}
	...
	private void FinalizeNpcList() {
		...
		foreach (var kvp in mapENpcCache) {
			...
			if (cacheOb is { gameLogicOb: not null }) {
				if (cacheOb.mapId != 0) {
				...
				// normal and annoying.
				// Plugin.Log.Info($"Failed to add triad[{cacheOb.triadId}], enpc[{kvp.Key}], name:{cacheOb.gameLogicOb.Name.GetLocalized()} - no location found!");
		...
		foreach (var kvp in mapENpcCache) {
			...
			if (!mapNpcAchievementId.TryGetValue(kvp.Value.triadId, out gameNpcOb.achievementId)) {
				Plugin.Log.Info($"Failed to find achievId for triadId:{kvp.Value.triadId}");
			}

			gameNpcOb.matchFee = kvp.Value.matchFee;
			if (kvp.Value.mapCoords != null) {
				gameNpcOb.Location = new MapLinkPayload(kvp.Value.territoryId, kvp.Value.mapId, kvp.Value.mapCoords[0], kvp.Value.mapCoords[1]);
			}
			...
			foreach (var cardId in kvp.Value.rewardCardIds) {
				...
				if (cardInfo == null) {
					Plugin.Log.Error($"Failed to match npc reward data! npc:{gameNpcOb.npcId}, key:{kvp.Key}, card:{cardId}");
					continue;
				}
				...
				if (npcOb != null) {
					...
					Plugin.Log.Error($"Failed to match npc reward data! npc:{gameNpcOb.npcId}, key:{kvp.Key}");
				}
			}
	...
	private static void FixLocalizedNameCasing() {
		var locDB = LocalizationDB.Get();
		var excludedList = new[] { "the", "goe", "van", "des", "sas", "yae", "tol", "der", "rem" };

		var fixLists = new[] { locDB.LocCardNames, locDB.LocNpcNames };
		foreach (var list in fixLists) {
			foreach (var entry in list) {
				...
				if (numChangedTokens > 0) {
					...
					//Plugin.Log.Info($"fixed casing: '{entry.Text}' => '{newText}'");
					entry.Text = newText;
				}
			}
		}

...
public class GameUIParser {
	...
	public void OnFailedCard(string desc) {
		Plugin.Log.Error($"failed to match card: {desc}");
		hasFailedCard = true;
	}
	...
	public void OnFailedModifier(string desc) {
		Plugin.Log.Error($"failed to match rule: {desc}");
	...
	public TriadNpc? ParseNpcNameStart(string desc, bool markFailed = true) {
		...
		var matchPattern = desc.Length > PartialNpcNameLength ? desc.Substring(0, PartialNpcNameLength) : desc;
	...
	public void OnFailedNpc(string desc) {
		Plugin.Log.Error($"failed to match npc: {string.Join(", ", desc)}");
		hasFailedNpc = true;
	}
	...
	public void OnFailedNpcSilent(string desc) {
		...
		if (lastLoggedNpc != desc) {
			lastLoggedNpc = desc;
			Plugin.Log.Info($"failed to match npc: {string.Join(", ", desc)}, is it pvp?");
		}

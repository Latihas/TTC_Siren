...
public class TriadGameScreenMemory {
	...
	private static bool IsDeckMatching(TriadDeckInstanceScreen deckInstance, TriadCard[] cards) {
	...
	private void UpdateAvailableRedCards(TriadDeckInstanceScreen redDeck,
		...
		if (bCanCompareWithPrevData) {
			...
			var usedCardsIndices = new List<int>();
			var usedCardsOther = new List<TriadCard>();
	...
	private static bool FindSwappedCard(TriadCard[] screenCards, TriadCard[] expectedCards, TriadDeckInstanceScreen otherDeck, out int swappedCardIdx, out int swappedOtherIdx, out TriadCard swappedCard) {
		swappedCardIdx = -1;
		swappedOtherIdx = -1;
		swappedCard = null;
	...
	private static bool FindSwappedCardVisible(TriadCard[] screenCards, TriadCardInstance[] board, TriadDeckInstanceScreen otherDeck, out int swappedCardIdx, out int swappedOtherIdx, out TriadCard swappedCard) {
		swappedCardIdx = -1;
		swappedOtherIdx = -1;
		swappedCard = null;
	...
	private static TriadCard[] FindCommonCards(List<TriadCard[]> deckHistory) {
		...
		if (deckHistory.Count > 1) {
			...
			for (var SlotIdx = 0; SlotIdx < result.Length; SlotIdx++) {
				var slotCounter = new Dictionary<TriadCard, int>();
	...
	private EUpdateFlags DetectSwapOnGameStart() {
		...
		var blueSwappedCardIdx = -1;
		var redSwappedCardIdx = -1;
		TriadCard blueSwappedCard = null;
		var bHasSwappedCard = FindSwappedCardVisible(deckBlue.cards, gameState.board, deckRed, out blueSwappedCardIdx, out redSwappedCardIdx, out blueSwappedCard);
		if (!bHasSwappedCard) {

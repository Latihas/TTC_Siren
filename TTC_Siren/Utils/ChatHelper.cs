...
public sealed unsafe class ChatHelper {
	...
	static ChatHelper() {
		ProcessChatBox ??=
			Marshal.GetDelegateForFunctionPointer<ProcessChatBoxDelegate>(Plugin.SigScanner.ScanText("48 89 5C 24 ?? 48 89 74 24 ?? 57 48 83 EC 20 48 8B F2 48 8B F9 45 84 C9"));
	}
	...
	public void SendMessage(string message) {
		if (string.IsNullOrEmpty(message)) {
			Plugin.Log.Error("待发送消息为空");
			return;
		}

		if (message.Length != SanitiseText(message).Length) {
			Plugin.Log.Error($"待发送消息中包含非法字符:\n{message}");
			return;
		}

		ReadOnlySpan<byte> bytes = Encoding.UTF8.GetBytes(message);
		if (bytes.Length > 500) {
			Plugin.Log.Error($"待发送消息不能长于 500 字节:\n{message}");
			return;
		}
